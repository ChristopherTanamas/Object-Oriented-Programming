...
class Kiosk:
    def run(self, ilkino, choice):
        while True:
            ...
            if choice == "1":
                ...
                self.book(ilkino, name, seat_numbers)
            elif choice == "2":
                name = input("Enter the name to find bookings: ")
                self.search(ilkino, name)
            elif choice == "3":
    ...
    def book(self, ilkino: Ilkino, name, seat_numbers):
        ...
        for num in seat_numbers:
            ...
            if isinstance(all_seats[num - 1], SpecialSeat) and all_seats[num - 1].get_availability():
                ...
                return f"Seats {num} have been booked or are not available."
    ...
    def search(self, ilkino:Ilkino, name):
        booked_seats = []
        string_seats = ''
        ...
        if len(booked_seats) != 0:                   
            for i in range(len(booked_seats)):
                if i == len(booked_seats)-1:
                    ...
                    string_seats += str(booked_seats[i]) + ', '
            return f"Seats {string_seats} are booked by {name}."
        else:
